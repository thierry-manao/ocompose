...
CI3_DOCKER_DB
        ...
        # Set ENVIRONMENT = 'docker' in index.php so CI3 loads application/config/docker/*
        # CI3's index.php has: define('ENVIRONMENT', isset($_SERVER['CI_ENV']) ? ... : 'development')
        # The cleanest way: set CI_ENV server var in the prepend file.
        cat >> "$prepend_file" <<'PREPEND_ENVIRONMENT'

// ── Set CI3 environment to 'docker' so it loads config/docker/ overrides ──
if (!isset($_SERVER['CI_ENV'])) {
    $_SERVER['CI_ENV'] = 'docker';
}
PREPEND_ENVIRONMENT
