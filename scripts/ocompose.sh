...
# ── Generate docker-compose.override.yml based on environment flags ──
generate_docker_override() {
    local instance_dir="$INSTANCES_DIR/$INSTANCE"
    local override_file="$instance_dir/docker-compose.override.yml"

    # Check if any override features are enabled
    local override_configs="${CI3_OVERRIDE_CONFIGS:-false}"
    local override_nginx="${OVERRIDE_NGINX_CONFIG:-false}"
    local custom_session_path="${CI3_CUSTOM_SESSION_PATH:-}"
    local connect_dbserver="${CONNECT_TO_DBSERVER:-false}"
    local app_type="${CI3_APP_TYPE:-single}"

    # If nothing is enabled, remove any auto-generated override file and return
    if [[ "$override_configs" == "false" && "$override_nginx" == "false" && \
          -z "$custom_session_path" && "$connect_dbserver" == "false" ]]; then
        if [[ -f "$override_file" ]]; then
            # Only remove if it's auto-generated (contains our marker)
            if grep -q "# Auto-generated by ocompose" "$override_file" 2>/dev/null; then
                rm -f "$override_file"
                log_verbose "   ${CYAN}✓${NC} Removed auto-generated docker-compose.override.yml"
            fi
        fi
        return 0
    fi

    log_verbose "${CYAN}⚙️  Generating docker-compose.override.yml...${NC}"

    # Start building the override file
    cat > "$override_file" <<'OVERRIDE_HEADER'
# Auto-generated by ocompose from .env settings
# DO NOT EDIT - Changes will be overwritten on next 'up' or 'restart'
# To customize manually, set all CI3_OVERRIDE_* and OVERRIDE_* flags to false

services:
OVERRIDE_HEADER

    # ── PHP service overrides ──
    local php_section_needed="false"
    local php_volumes=""
    local php_networks=""

    # CI3 config overrides
    if [[ "$override_configs" == "true" ]]; then
        php_section_needed="true"
        php_volumes+="      # CodeIgniter 3 config overrides"$'\n'
        php_volumes+="      - ./instances/\${PROJECT_NAME}/docker/config/database.php:/var/www/html/application/config/development/database.php"$'\n'
        php_volumes+="      - ./instances/\${PROJECT_NAME}/docker/config/config.php:/var/www/html/application/config/development/config.php"$'\n'
        php_volumes+="      - ./instances/\${PROJECT_NAME}/docker/config/constants.php:/var/www/html/application/config/development/constants.php"$'\n'
        php_volumes+="      # Writable volumes (logs, cache) - prevents permission errors"$'\n'
        php_volumes+="      - app-logs:/var/www/html/application/logs"$'\n'
        php_volumes+="      - app-cache:/var/www/html/application/cache"$'\n'

        if [[ "$app_type" == "dual-webservice" ]]; then
            php_volumes+="      # Webservice configs (shared)"$'\n'
            php_volumes+="      - ./instances/\${PROJECT_NAME}/docker/config/database.php:/var/www/html/webservice/v1/application/config/development/database.php"$'\n'
            php_volumes+="      - webservice-logs:/var/www/html/webservice/v1/application/logs"$'\n'
        elif [[ "$app_type" == "dual-api" ]]; then
            php_volumes+="      # API configs (separate)"$'\n'
            php_volumes+="      - ./instances/\${PROJECT_NAME}/docker/config/api-database.php:/var/www/html/api/application/config/development/database.php"$'\n'
            php_volumes+="      - ./instances/\${PROJECT_NAME}/docker/config/api-config.php:/var/www/html/api/application/config/development/config.php"$'\n'
            php_volumes+="      - ./instances/\${PROJECT_NAME}/docker/config/api-constants.php:/var/www/html/api/application/config/development/constants.php"$'\n'
            php_volumes+="      - api-logs:/var/www/html/api/application/logs"$'\n'
        fi
    fi

    # Custom session path
    if [[ -n "$custom_session_path" ]]; then
        php_section_needed="true"
        php_volumes+="      # Custom session volume"$'\n'
        php_volumes+="      - sessions:${custom_session_path}"$'\n'
    fi

    # External network connection
    if [[ "$connect_dbserver" == "true" ]]; then
        php_section_needed="true"
        php_networks+="      - net"$'\n'
        php_networks+="      - dbserver"$'\n'
    fi

    # Write PHP section if needed
    if [[ "$php_section_needed" == "true" ]]; then
        echo "  php:" >> "$override_file"
        if [[ -n "$php_volumes" ]]; then
            echo "    volumes:" >> "$override_file"
            echo -n "$php_volumes" >> "$override_file"
        fi
        if [[ -n "$php_networks" ]]; then
            echo "    networks:" >> "$override_file"
            echo -n "$php_networks" >> "$override_file"
        fi
        echo "" >> "$override_file"
    fi

    # ── NGINX service overrides ──
    local nginx_section_needed="false"
    local nginx_volumes=""
    local nginx_networks=""

    # Custom nginx config
    if [[ "$override_nginx" == "true" ]]; then
        nginx_section_needed="true"
        nginx_volumes+="      - ./instances/\${PROJECT_NAME}/docker/nginx/default.conf:/etc/nginx/conf.d/default.conf:ro"$'\n'
    fi
    ...
    if [[ "$connect_dbserver" == "true" ]]; then
        nginx_section_needed="true"
        nginx_networks+="      - net"$'\n'
        nginx_networks+="      - dbserver"$'\n'
    fi

    # Write nginx section if needed
    if [[ "$nginx_section_needed" == "true" ]]; then
        echo "  nginx:" >> "$override_file"
        if [[ -n "$nginx_volumes" ]]; then
            echo "    volumes:" >> "$override_file"
            echo -n "$nginx_volumes" >> "$override_file"
        fi
        if [[ -n "$nginx_networks" ]]; then
            echo "    networks:" >> "$override_file"
            echo -n "$nginx_networks" >> "$override_file"
        fi
        echo "" >> "$override_file"
    fi

    # ── Volumes section ──
    local volumes_needed="false"
    local volumes_content=""

    if [[ -n "$custom_session_path" ]]; then
        volumes_needed="true"
        volumes_content+="  sessions:"$'\n'
    fi

    if [[ "$override_configs" == "true" ]]; then
        volumes_needed="true"
        volumes_content+="  app-logs:"$'\n'
        volumes_content+="  app-cache:"$'\n'

        if [[ "$app_type" == "dual-webservice" ]]; then
            volumes_content+="  webservice-logs:"$'\n'
        elif [[ "$app_type" == "dual-api" ]]; then
            volumes_content+="  api-logs:"$'\n'
        fi
    fi

    if [[ "$volumes_needed" == "true" ]]; then
        echo "volumes:" >> "$override_file"
        echo -n "$volumes_content" >> "$override_file"
        echo "" >> "$override_file"
    fi

    # ── Networks section ──
    if [[ "$connect_dbserver" == "true" ]]; then
        cat >> "$override_file" <<'OVERRIDE_NETWORKS'
networks:
  dbserver:
    external: true
...
OVERRIDE_NETWORKS
    ...
    log_verbose "   ${GREEN}✓${NC} Generated docker-compose.override.yml"
}
