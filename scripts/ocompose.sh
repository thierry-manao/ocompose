...
PREPEND_CI_CONFIG
        ...
        # Generate the CI3 config override file (loaded by CI3 via environment config)
        local ci3_env_config_dir="$ci3_root/application/config/docker"
        mkdir -p "$ci3_env_config_dir"

        cat > "$ci3_env_config_dir/config.php" <<'CI3_DOCKER_CONFIG'
<?php
defined('BASEPATH') OR exit('No direct script access allowed');
/**
 * Auto-generated by ocompose — do not edit manually.
 * CI3 environment-specific config override (ENVIRONMENT = 'docker').
 * Only the overridden keys are set here; everything else comes from
 * the main application/config/config.php.
 */
...
/**
 * Auto-generated by ocompose — do not edit manually.
 * CI3 environment-specific database override (ENVIRONMENT = 'docker').
 */
...
CI3_DOCKER_DB
        ...
        # Set ENVIRONMENT = 'docker' so CI3 loads application/config/docker/*
        # Many CI3 apps hardcode ENVIRONMENT via HTTP_HOST checks instead of
        # using CI_ENV.  Since auto_prepend_file runs before index.php, defining
        # the ENVIRONMENT constant here locks it in.  When index.php later tries
        # define('ENVIRONMENT', ...) it's silently skipped (already defined).
        cat >> "$prepend_file" <<'PREPEND_ENVIRONMENT'

// ── Force CI3 environment to 'docker' ──
...
PREPEND_ENVIRONMENT
        ...
        # Generate docker/constants.php — chains to development/constants.php
        # so all app-specific constants are loaded, while DB/path constants
        # (BDD_HOST, BDD_USER, BDD_PWD, APP_ROOT) are already locked in by
        # the prepend file and silently win.
        cat > "$ci3_env_config_dir/constants.php" <<'CI3_DOCKER_CONSTANTS'
<?php
defined('BASEPATH') OR exit('No direct script access allowed');
/**
 * Auto-generated by ocompose — do not edit manually.
 * Loads the development constants so all app-specific defines are available.
 ...
 */
$_oc_dev_constants = APPPATH . 'config/development/constants.php';
if (is_file($_oc_dev_constants)) {
    require_once $_oc_dev_constants;
}
unset($_oc_dev_constants);
CI3_DOCKER_CONSTANTS

        echo -e "   ${GREEN}✓${NC} Generated ${prepend_file#"$INSTANCES_DIR/$INSTANCE/"}"
        echo -e "   ${GREEN}✓${NC} Generated application/config/docker/ overrides"
    done
