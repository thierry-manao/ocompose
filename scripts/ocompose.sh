...
PREPEND_CI_CONFIG
        ...
        # index.php switch statement, and config/development/ is typically
        # gitignored (contains local secrets).  Files are only written if they
        # don't already exist, to avoid overwriting user customizations.
        local ci3_env_config_dir="$ci3_root/application/config/development"
        mkdir -p "$ci3_env_config_dir"

        # config.php — applies base_url / sess_save_path from the prepend globals
...
/**
 * Auto-generated by ocompose — do not edit manually.
 * CI3 environment-specific config override (ENVIRONMENT = 'development').
...
CI3_DOCKER_CONFIG
        ...
        # database.php — connects to the Docker DB container
        if [[ ! -f "$ci3_env_config_dir/database.php" ]]; then
            cat > "$ci3_env_config_dir/database.php" <<CI3_DOCKER_DB
<?php
defined('BASEPATH') OR exit('No direct script access allowed');
/**
 * Auto-generated by ocompose — do not edit manually.
 * CI3 environment-specific database override (ENVIRONMENT = 'development').
 */
